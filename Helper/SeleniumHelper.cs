...

{
    public class SeleniumHelper
    {
            ...
            if (DriverHelpers.AreYouBrowserstackdriver.Equals(true))
            {
                if (TestContext.CurrentContext.Result.Outcome.Status.Equals(TestStatus.Failed))
                {
                    ...
                    ((IJavaScriptExecutor)_webDriver).ExecuteScript(
                        ...
                        strPresentDateTime + " " + TestContext.CurrentContext.Test.MethodName?.ToString() + "\"}}");
                }
                else
                {
                    ((IJavaScriptExecutor)_webDriver).ExecuteScript(
                        "browserstack_executor: {\"action\": \"setSessionStatus\", \"arguments\": {\"status\":\"passed\", \"reason\": \"PASSED WITHOUT ERRORS\"}}");
                    ((IJavaScriptExecutor)_webDriver).ExecuteScript(
                        "browserstack_executor: {\"action\": \"setSessionName\", \"arguments\": {\"name\":\"" +
                        strPresentDateTime + " " + TestContext.CurrentContext.Test.MethodName?.ToString() + "\"}}");
                }
        ...
        {
            bool jqueryDefined =
                (bool)((IJavaScriptExecutor)_webDriver).ExecuteScript("return typeof jQuery != 'undefined'");
            if (jqueryDefined)
            {
                WaitUntilJQueryLoad();
            }
        ...
        public void WaitUntilJQueryLoad()
        {
            ...
            fluentWait.Until(driver =>
            {
                ...
                {
                    bool jqueryReady =
                        (bool)((IJavaScriptExecutor)_webDriver).ExecuteScript("return jQuery.active == 0");
                    if (jqueryReady)
                    {
                        return true;
                    }
